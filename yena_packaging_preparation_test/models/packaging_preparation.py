...
class PackagingDeliveryReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        headers = ['Purchase Order No', 'Drawing Number', 'Quantity','Number of Euro-Pallets','Is Unique?','Total Quantity']
...
class PackagingBillOfLadingReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        for batch in batches:
            ...
            # Her bir teslim adresi için bilgileri yazdırın ve adres hücrelerini birleştirin
            for address, infos in addresses.items():
                start_row = row
                for info in infos:
                    sheet.merge_range(row, 2,row,5, info, merge_format10)  # Kap bilgileri 2. sütundan itibaren yazılır
                    row += 1
                end_row = start_row + len(infos) - 1
                sheet.merge_range(start_row, 0, end_row, 1, address, merge_format12)
