...
class PackagingPreparationDeliveryReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        # Eğer blanket order line mevcutsa, ilgili sütunu ekle
        ...
        grouped_data = defaultdict(lambda: {'total_quantity': 0, 'total_pallets': 0, 'product_name': ''})
        hs_codes = set()
        for batch in batches:
            ...
            for prep in batch.packaging_preparation_ids:
                ...
                hs_code = prep.product_id.hs_code
                if hs_code:
                    hs_codes.add(hs_code)
                    
                customer_reference = str(prep.customer_reference) if prep.customer_reference else ""
                product_id = str(prep.product_id.id) if prep.product_id else ""
                key = (customer_reference, product_id)
                grouped_data[key]['total_quantity'] += prep.package_quantity or 0
                grouped_data[key]['total_pallets'] += 1  # Her bir `prep` için palet sayısını 1 arttır
                grouped_data[key]['product_name'] = prep.product_id.display_name or ''

        # Gruplanmış verileri sırala ve Excel'e yaz
        sorted_keys = sorted(grouped_data.keys(), key=lambda x: (x[0], x[1]))

        for key in sorted_keys:
            data = grouped_data[key]
            sheet.write(row, start_col, key[0], centered_format)  # Customer reference
            sheet.write(row, start_col + 1, data['product_name'], centered_format)  # Ürün adı
            sheet.write(row, start_col + 2, data['total_quantity'], centered_format)  # Toplam miktar
            sheet.write(row, start_col + 3, data['total_pallets'], centered_format)  # Toplam palet sayısı
            row += 1

        sheet.write(row, start_col + 2, 'Total Pallets:', merge_format)
        sheet.write(row, start_col + 3, sum(data['total_pallets'] for data in grouped_data.values()), merge_format)
        
        sheet.merge_range("F24:G24", "Tariff Number: ", centered_format)
        sheet.merge_range("H24:K24", ', '.join(hs_codes), merge_format)
        sheet.merge_range("F26:K26",f"Delivery Date (ETA): {arrival_date}",merge_format)
...
class PackagingBillOfLadingReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batches):
        ...
        for batch in batches:
            ...
            address_lines = []
            for receiveFrom in batch.vendor_ids:
                ...
                address_lines.append(f"Alım Adresi: {receive_address}")
            ...
            for customer in batch.customer_ids:
                ...
                address_lines.append(f"Teslim Adresi: {delivery_address}")
                    ...
                    for volume, count in volumes.items():
                        for pallet_no, data in volume_by_pallet_customer_specific.items():
                            ...
                            if calculated_volume == volume:
                                ...
                                customer_pallet_counts.append(f"{customer_name}: {count} Kap, {volume_text}, {stackable_text}")
            ...
            sheet.merge_range('C7:D7', "Bill of Lading", header_format)
            sheet.merge_range('E7:F7', date_now, header_format)
            sheet.merge_range('B8:E8', '', border_format1)
    
            sheet.merge_range('A9:B11', "Alıcı Firma:", merge_format12)
            sheet.merge_range('C9:E11', "YENA ENGINEERING B.V.\nWorld Trade Center Rotterdam Beursplein 37 3011 AA Rotterdam/Netherlands", merge_format10)

            sheet.merge_range('A12:B13', "Gönderen Firma:", merge_format12)
            sheet.merge_range('C12:E13', "YENA DEMİR ÇELİK SAN. VE TİC. LTD ŞTİ\nOsmanağa Mah. Rıhtım Caddesi No:8/5 34714 Kadıköy/İstanbul", merge_format10)
            
            sheet.merge_range('A14:B15', "Varış Gümrüğü", merge_format12)
            sheet.merge_range('C14:E15', f"{logistic_company} tarafından", merge_format10)

            sheet.merge_range('A24:B25','Araç Talebi',merge_format12)
            sheet.merge_range('C24:E25',vehicle_type,merge_format10)
            
            sheet.merge_range('A26:B27', 'Description of Products', merge_format12)
            sheet.merge_range('C26:E27' ,', '.join(product_descriptions), merge_format10)
           
            sheet.merge_range('A28:B33' , "Kap Adedi ve Ebadı", merge_format12)

            sheet.merge_range('A34:B35', "AMBALAJLANMA", merge_format12)
            sheet.merge_range('C34:E35', total_pallet_count, merge_format10)
            
            sheet.merge_range('A36:B37', "Yükleme Tarihi", merge_format12)
            sheet.merge_range('C36:E37', edespatch_date_str, merge_format10)
            sheet.merge_range('A38:B39', "Net/Brüt Ağırlık", merge_format12)
            sheet.merge_range('C38:E39', f"Net Ağırlık: {total_net_weight}KG\nBrüt Ağırlık:{total_gross_weight}KG", merge_format10)
            sheet.merge_range('A40:B41', "Malzemenin Teslim Şekli", merge_format12)
            sheet.merge_range('C40:E41',"DAP,NETHERLANDS" , merge_format10)
            sheet.merge_range('A42:B43', "Gümrükçü Firma", merge_format12)
            sheet.merge_range('C42:E43',"TÜRKİYE GÜMRÜK: MAVİPORT" , merge_format10)
            
            sheet.merge_range('A44:D44', '', border_format2)
            sheet.merge_range('A45:D45', "YENA DEMİR ÇELİK SAN. VE TİC. LTD ŞTİ", bottom_text)
            sheet.merge_range('A46:D46', "Osmanağa Mah. Rıhtım Caddesi No:8/5 34714 Kadıköy/İstanbul", merge_format11)

            sheet.merge_range('A16:B23' , "Alım/Teslim Adresi", merge_format12)
            row=15
            start_col=0
            final_text = "\n".join(customer_pallet_counts)
      
            sheet.merge_range('C28:E33', final_text, merge_format10)
            addresses = "\n".join(address_lines)
            sheet.merge_range(row, start_col + 2, 22, start_col + 4, addresses, merge_format10)
